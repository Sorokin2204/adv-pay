...
const GamePage = () => {
  ...
  const { id } = useParams();
  const [serverGame4, setServerGame4] = useState([]);
  useEffect(() => {
    axios.get('https://payment.lotr-risetowar.com/api/v1/base_info').then((res) => {
      setServerGame4(res.data.data.server.mobile?.map((ser) => ({ name: ser.name, serverId: ser.id })));
    });
  }, []);

  console.log(serverGame4);
  const [gameList, setGameList] = useState([]);
  useEffect(() => {
    if (serverGame4?.length !== 0) {
      const games = [
        {
          id: 1,
        ...
        {
          id: 3,
          name: 'Донат в игру Onmyoji Arena',
          desc: (
            <>
              <ul className="game__list">
                <li className="game__list-item"> На данной странице вы донатите в игру Onmyoji Arena самостоятельно, указывая только ID</li>
                <li className="game__list-item"> Донаты абсолютно легальны, наш сайт автоматизирует пополнение через официальных поставщиков (Codashop, RazerGold и др.)</li>
                <li className="game__list-item"> С каждым паком вам приходит бонусная игровая валюта</li>
                <li className="game__list-item"> Донаты приходят моментально</li>
                <li className="game__list-item"> Если возникла ошибка при донате, повторите попытку или напишите нам</li>
                <li className="game__list-item"> Мы не несем ответственности, если вы указали неверный ID</li>
                <li className="game__list-item"> Деньги сначала зачисляются на ваш лицевой счет сайта, потом вы их донатите в игру</li>
              </ul>
        ...
        {
          id: 4,
          name: 'Донат в игру Властелин колец',
          desc: (
            <>
              <ul className="game__list">
                <li className="game__list-item"> На данной странице вы донатите в игру Onmyoji Arena самостоятельно, указывая только ID</li>
  ...
  useEffect(() => {
    if (dataFindTypeGame && !errorFindTypeGame && gameList?.length !== 0) {
      const findGame = gameList?.find((gameItem) => gameItem?.id === dataFindTypeGame?.id);
      if (findGame) {
        setDataPage(findGame);
      }
    }
  }, [dataFindTypeGame, gameList]);
  return gameList?.length !== 0 ? <DrawerAppBar isFull>{dataFindTypeGame && dataPage ? <GamePageComponent data={dataPage} /> : errorFindTypeGame ? <NotFound /> : <></>}</DrawerAppBar> : <></>;
};
